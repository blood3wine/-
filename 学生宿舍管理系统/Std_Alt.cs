...
namespace 学生宿舍管理系统
{
    public partial class Std_Alt : Form
    {
        ...
        {
            string Std_Alt_connString = "server=DESKTOP-V3H7EOU\\MSSQLSERVER02;database=Stu_Doemitory; Trusted_Connection =SSPI";
            SqlConnection Std_Alt_conn = new SqlConnection(Std_Alt_connString);

            string Std_Alt_sql = "update student set dep_ID = '" + Std_Alt_DepID.Text + "',class = '" + Std_Alt_Class.Text + "',sname = '" + Std_Alt_Sname.Text + "',sex = '" + Std_Alt_Sex.Text + "',age = '" + Std_Alt_Age.Text + "',tel = '" + Std_Alt_Tel.Text + "',dor_ID = '" + Std_Alt_DorID.Text + "',InTime = '" + Std_Alt_InTime.Text + "'where sno = '"+Std_Alt_Sno.Text+"'";
            try
            {
                Std_Alt_conn.Open();
