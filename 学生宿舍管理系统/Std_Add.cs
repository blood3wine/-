...
namespace 学生宿舍管理系统
{
    public partial class Std_Add : Form
    {
        ...
        {
            string Std_Add_connString = "server=DESKTOP-V3H7EOU\\MSSQLSERVER02;database=Stu_Doemitory; Trusted_Connection =SSPI";
            SqlConnection Std_Add_conn = new SqlConnection(Std_Add_connString);

            string Std_Add_sql = "insert into student(dep_ID,sno,sname,sex,age,dor_ID,class,InTime,tel) values('" + Std_Add_DepNum.Text + "','" + Std_Add_Sno.Text + "','" + Std_Add_Sname.Text + "','" + Std_Add_Sex.Text + "','" + Std_Add_Sage.Text + "','" + Std_Add_DorID.Text + "','" + Std_Add_Class.Text + "','" + Std_Add_InTime.Text + "','" + Std_Add_Tel.Text +  "')";
            try
            {
                Std_Add_conn.Open();
                ...
                {
                    MessageBox.Show("学生信息添加成功！宿舍相应人数修改成功！", "添加成功", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    Std_Add_DepNum.Text = "";
                    Std_Add_Sno.Text = "";
                    Std_Add_Sname.Text = "";
                    Std_Add_Sex.Text = "";
